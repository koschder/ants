...
Das Codepaket, welches von den Challenge-Organisatoren mitgeliefert wird, bietet bereits eine hilfreiche 2D-Visualisierung des Spiels, mit welcher das Spielgeschehen mitverfolgt werden kann. Die Visualisierung wurde mit HMTL und Javascript implementiert. Leider ist es nicht ohne weiteres möglich, zusätzliche Informationen auf die Seite zu projizieren. Deshalb haben wir den Viewer bereits im Projekt 2 mit einer solchen Funktion erweitert. Mit der Codezeile \texttt{LiveInfo.liveInfo(...)} kann eine Zusatzinformation geschrieben werden, welche wir später im Game-Viewer anzeigen können. Es muss definiert werden mit welchem Zug und wo auf dem Spielfeld die Information angezeigt werden soll. Im Beispiel wird an der Position der Ameise (\texttt{ant.getTile()}) ausgegeben, welchen Task die Ameise hat.
\lstset{language=Java, tabsize=4}
\begin{lstlisting}
LiveInfo.liveInfo(Ants.getAnts().getTurn(), ant.getTile(), "Task: %s ant: %s", issuer, ant.getTile());
\end{lstlisting}

Klick man nun auf der Karte während dem entsprechenden Zug auf die Zelle zu welcher eine zusätzliche Information gespeichert wurde, erscheint ein einfaches aber praktisches Popup mit den Informationen. Dank solcher Zusatzinformationen muss nicht mühsam im Log nachgeschaut werden, welcher Ameise wann und wo welcher Aufgabe nachgeht.
