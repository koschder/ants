...
Das erstellte Suchframework bietet eine Pfadsuche mit drei verschiedenen Algorithmen an. Zudem stellt es eine Breitensuche zur Verfügung und eine spezielle Anwendung davon, die Barriersuche. Als Ergänzung wurde auch ein PathSmoothing-Algorithmus implementiert.

\section{Entities für die Pfadsuche}
...
\caption{Spiel-Elemente für die Suche}
...
\item
\textbf{Cluster}: Der Cluster ist ein Kartenausschnitt und wird vom HPA* Algorithmus (Kapitel \ref{subsec:module.Suchalgorithmen.Pfadsuche.HPAstar}) genutzt, indem er die Pfade kennt, die durch diesen Kartenausschnitt führen.
\item
...
Das Interface PathFinder definiert eine einfache \texttt{search()} Methode, die die Angabe von Start, Ziel und einer Suchstrategie erlaubt. Die \texttt{update()} Methode ermöglicht ein Aktualisieren der Berechnungsgrundlagen für die Pfadsuche (z.B. das Clustering).

Die Klasse SimplePathFinder ist die Standard-Implementierung des PathFinder Interfaces und führt verschiedene Suchstrategien aus. Für den HPA* Algorithmus wurde diese Klasse durch ClusteringPathFinder erweitert. Diese beinhaltet zusätzlich, wie der Name schon sagt, das Clustering.

\section{Pfadsuche}
\label{sec:module.Suchalgorithmen.Pfadsuche}
Wir haben drei unterschiedliche Pfadsuchalgorithmen in unserem Code eingebaut. Via PathFinder-Klasse wird für die Pfadsuche der Algorithmus ausgewählt.
...
Den A* Algorithmus haben wir nach dem Beschrieb im Buch \cite{AIMA} implementiert. A* ist wahrscheinlich der meist verbreitete und bekannteste heuristische Suchalgorithmus, und hat sich bereits vielfach bewährt.
...
Die Abbildung \ref{fig:heuristicAstar} zeigt den effektiven Pfad (grau) vom zu expandierenden roten Knoten mit den minimalen Kosten von 10 \gls{Tile}s. Die Luftlinie (blau) als heuristischer Wert hat aber nur eine Länge von 7.6 \gls{Tile}s. Damit erfüllt unsere Heuristik die Anforderungen des Algorithmus.
...
Eine Pfadsuche mit A* wird gleich ausgelöst wie die Suche mit dem Simple-Algorithmus, ausser dass als Parameter die Strategy \texttt{AStar} gewählt wird.
...
Eine Pfadsuche A* über alle Zellen der Spielkarte ist ziemlich teuer, da es viele Pfade gibt, die zum Teil nur ein oder wenige \gls{Tile}s nebeneinander liegen. Es wird bis zum Schluss verschiedenen Pfaden nachgegangen, die sehr ähnlich sind. Abhilfe zu dieser sehr feinmaschigen Pfadsuche bietet das Hierarchical Pathfinding A* bei welchem die Karte in Regionen (Cluster) aufgeteilt wird. Von Cluster zu Cluster werden Verbindungspfade vorberechnet, welche der Algorithmus bei der Pfadsuche verwendet.

\subsubsection{Warum HPA*?}
Die Suche mit A* geht so lange gut, wie wir noch nicht so viele Ameisen haben und uns nur beschränkt auf der Karte bewegen. Sobald aber das Spiel fortgeschritten ist, wir viele Ameisen haben und uns auf der ganzen Karte bewegen, geht zu viel Zeit in der Pfadsuche verloren. Dies wollten wir verbessern und stiessen auf den HPA* Algorithmus. Dieser passt gut in unser Spiel, denn so können wir in den Anfangsphasen des Spiel, wo wir noch Zeit zur Verfügung haben, die Pfadabschnitte dank dem Clustering vorberechnen und so später im Spiel diese Zeit bei der Berechnung sparen.\\
Die Implementation erfolgte in Anlehnung an das Paper \cite{nohpa:IM}, wobei sich im Gegensatz zum Paper unsere Clusters um eine Zelle überschneiden. Zudem wird im Paper nur die Clusterart die wir später ''Corner`` nennen beschrieben. Die Clusterart ''Centered`` haben wir dazu programmiert.

\subsubsection{Clustering}
Das Clustering wird während dem ClusteringTask (siehe \ref{sec:module.Tasks}) ausgeführt, Dabei wird die Spielkarte, wie bereits erwähnt, in sogenannte Clusters unterteilt. Auf der Abbildung \ref{fig.clusteredMap} wurde die Karte in 16 Clusters aufgeteilt.

\begin{figure}[H]
\centering
\includegraphics[height=50mm]{91_bilder/clusteredMap}
\caption[Clustereinteilung auf der Landkarte]{Clustereinteilung auf der Landkarte. Clustergrösse 4x4, Spielkarte 16x16}
\label{fig.clusteredMap}
\end{figure}

Wir unterschieden zwischen den zwei Clusterarten \texttt{Centered} und \texttt{Corner}. Die Variante \texttt{Corner} wurde bereits im Vormodule ''Projekt 2`` implementiert, während die Variante Center im Laufe dieser Arbeit dazu kam. Folgendes Bild zeigt den Unterschied der Varianten. \texttt{Corner} generiert zwei Übergangspunkte plus eine Verbindung auf der Kante zwischen zwei Clusters. Die Variante \texttt{Centered} generiert nur einen Übergangspunkt in der Kantenmitte der aneinander grenzenden Clusters. Die Variante \texttt{Centered} hat den Vorteil, dass es ein weniger dichtes Pfadnetz gibt, da weniger Übergangspunkte erzeugt werden, aber sie hat auch den Nachteil, dass der gefundene Pfad meistens länger ist als der \texttt{Corner}-Pfad. Die gefundenen Pfade sind meistens nicht optimal, daher empfiehlt es sich, zusätzlich ein Pathsmoothing anzuwenden.
...
Wenn ein Cluster zwei oder mehrere Aussenkanten kennt, berechnet er die Innenkanten mittels A*. Diese verbinden die Knoten der verschiedenen Aussenkanten. Das ergibt nun ein Pfadnetz über die Gesamtkarte. Im nachfolgenden Bild sind die Innenkanten (gelb) ersichtlich, die bei den ersten 8 Clustern berechnet wurden.
...
Angenommen, das Clustering wurde über die ganze Karte vorgenommen, kann wie in der Abbildung \ref{fig.clusteredMap4} ersichtlich ein Pfad vom \gls{Tile} <3,9> nach <13,9> (grüne Punkte) mittels HPA* gesucht  werden. Zuerst wird eruiert, in welchem Cluster sich das Start- bzw Zieltile befindet. Danach wird in dem gefundenen Cluster ein Weg zu einem beliebigen Knoten auf der Clusterseite gesucht. Sind diese Knoten erreicht (blaue Pfade), wird nun das vorberechnete Pfadnetz mittels A* Heuristik verwendet, um die beiden Knoten auf dem kürzesten möglichen Pfad (gelb) zu verbinden. Der resultierende Pfad könnte nun via Pathsmoothing noch verkürzt werden.
...
Um eine Pfadsuche mit dem  HPA* Alogrithmus durchzuführen, muss ein ClusteringPathFinder instanziert werden. Als Parameter erwartet der Konstruktor die Karte, auf welcher das Clustering und die Pfadsuche gemacht wird, sowie die Clustergrösse (hier: 10) und den Clustertyp. Das Clustering wird mit \texttt{pf.update()} durchgeführt. Danach kann die Pfadsuche gemacht werden. Falls nicht alle nötigen Clusters zur Verfügung stehen, weil es noch unbekannte Flecke auf der Karte gibt, wird als Ausweichlösung versucht, mit A* einen Pfad zu finden.
...
Die \gls{InfluenceMap}, welche wir während der Bachelorarbeit neu implementiert haben, kann auch für die Pfadsuche verwendet werden. Dabei sind die Pfadkosten für Gebiete, die vom Gegner kontrolliert sind höher als für neutrale Gebiete, und tiefer für Gebiete, die von unseren Ameisen kontrolliert werden. (Details zur Implementierung der \gls{InfluenceMap} siehe Kapitel \ref{sec:module.InfluenceMap}). 

Die Methode \texttt{getActualCost()} in der Klasse SearchStrategy wurde erweitert. Falls die Suche mit einer InfluenceMap initialisiert wurde, sind die Kosten nicht eine Einheit pro Pfadtile, sondern können zwischen 1 (sicheres Gebiet) und 4 (gefährliches Gebiet) Einheiten variieren. (Die Pfadkosten dürfen nicht negativ sein, sonst würde der A* Algorithmus nicht mehr korrekt funktionieren.) Die Kosten für jedes Pfadtile werden durch die Methode \texttt{getPathCosts()} der \gls{InfluenceMap} berechnet.
...
Dadurch resultiert ein Pfad, der eher durch sicheres Gebiet führt. Abbildungen \ref{fig.InfluenceAndPathfinding01} und \ref{fig.InfluenceAndPathfinding02}, welche durch einen UnitTest generiert wurden, bezeugen die korrekte Funktionalität. Der rote Punkt soll mit dem schwarzen Punkt durch einen Pfad verbunden werden. Auf der Karte sind zudem die eigenen, orangen Einheiten sowie die gegnerischen Einheiten (blau) abgebildet. Jede Einheit trägt mit ihrem Einfluss zur Berechnung der \gls{InfluenceMap} bei. Pro \gls{Tile} wird die Sicherheit auf der Karte ausgegeben, negativ für Gebiete die vom Gegner kontrolliert werden und positiv in unserem Hoheitsgebiet.
...
Ohne Berücksichtigung der \gls{InfluenceMap} würde der A* Algorithmus einen Pfad finden der auf direktem Weg waagrecht zum Zielpunkt führt. Sobald aber die \gls{InfluenceMap} berücksichtigt wird, führt der Pfad nicht mehr auf dem direktesten Weg zum Ziel, sondern nimmt einen Umweg über sicheres Gebiet. Unten abgebildet ist der kürzeste Pfad mit Berücksichtigung der \gls{InfluenceMap} (blau) und ohne \gls{InfluenceMap}-Berücksichtigung (orange). 

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{91_bilder/influenceAStar02.jpg}
\caption{Gefundene Pfade mit und ohne Berücksichtigung der \gls{InfluenceMap}}
\label{fig.InfluenceAndPathfinding02}
\end{figure}

Ein Vergleich der Pfadkosten macht deutlich, dass je nach Berücksichtigung der \gls{InfluenceMap} nicht der gleiche Pfad als der ''Kürzeste`` von A* gefunden wird.
...
\hline
 ...
 \end{tabular}
 \caption{Pfadkosten mit und ohne Berücksichtigung der \gls{InfluenceMap}}
\end{table}
 
\subsection{Pathsmoothing}
\label{subsec:module.Suchalgorithmen.Pfadsuche.Pathsmoothing}

Um unser Search-Framework zu komplettieren, bietet wir auch ein PathSmoothing, das ''Glätten`` eines Pfades an. Wie im Abschnitt Clustering schon erwähnt, kann es sein, dass ein Pfad, der vom HPA* Algorithmus gefunden wurde, nicht zwingend der Kürzeste ist. Abbildung \ref{fig.pathsmoothingOne} veranschaulicht, wie der gefundene Pfad von Cluster zu Cluster (weisse Markierung), stets über die vorberechneten Verbindungspunkte (blau) verläuft. Dies ist nicht der optimale Pfad, er kann mit PathSmoothing verkürzt werden.

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{91_bilder/pathsmoothingOne}
\caption{Der mit HPA* (Centered) gefundene Pfad ist nicht der kürzeste.}
\label{fig.pathsmoothingOne}
\end{figure}

Der Algorithmus des Pathsmoothing wurde nach \cite{ARTIFICIALINTELLIGENCEFORGAMES} programmiert und definiert sich (vereinfacht) wie folgt. Vom Pfad, der gekürzt werden soll, wird ein erster Abschnitt mit der Länge \texttt{size} genommen. Mittels manhattanDistance wird geprüft, ob ein kürzerer Weg für diesen Abschnitt möglich wäre. Falls ja wird mit A* ein neuer Pfad gesucht, sonst wird der alte Pfad (\texttt{subPath}) übernommen. Dieses Verfahren wird für alle nachfolgenden Pfadabschnitte gemacht, bis der ganze Pfad durchlaufen ist.

\paragraph{Rekursion:} Der beschriebene Algorithmus hat nicht in jedem Fall den kürzesten Pfad als Output. So sind zwar alle Pfadabschnitte optimal gekürzt, es kann aber sein, dass wenn zwei Abschnitte zusammen gefügt werden, der Pfad nicht mehr der kürzeste ist. Als Beispiel: Die Wegabschnitte Zürich-Thun und Thun-Genf mögen optimal gekürzt sein. Zusammengefügt zur Strecke Zürich-Genf, braucht es den Umweg über Thun nicht. Um den Umweg zu entfernen, wird der Algorithmus rekursiv aufgerufen, indem \texttt{smoothPath()} mit einer grösseren \texttt{size} als Parameter für die zusammengesetzten Abschnitte nochmals aufgerufen wird.
...
In Abbildung \ref{fig.pathsmoothingTwo} wurde der beschriebene PathSmoothing Algorithmus angewendet; dadurch konnte der Pfad von einer ursprünglichen Pfadlänge von 50 \gls{Tile}s (siehe \ref{fig.pathsmoothingOne}) auf eine Pfadlänge von 40 \gls{Tile}s reduziert werden.

\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{91_bilder/pathsmoothingTwo}
\caption{Der geglättete Pfad nach Anwendung des PathSmoothing}
