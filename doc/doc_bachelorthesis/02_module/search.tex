...
\item
\textbf{Cluster}: Der Cluster ist ein Kartenabschnitt und wird vom HPA* Algorithmus (Kapitel \ref{subsec:module.Suchalgorithmen.Pfadsuche.HPAstar}) genutzt, indem er die Pfade kennt, die durch diesen Kartenabschnitt führen.
\item
\textbf{Vertex}: Eine Vertex ist ein Knoten der Teil bei Pfadsuche ist. Er verbindet weitere Konten durch Katen (Edges).
\item
...
Die Klasse SimplePathFinder ist die Standard-Implementierung des PathFinder Interfaces und kann verschiedene Suchstrategien ausführen. Für den HPA* Algorithmus wurde diese Klasse durch ClusteringPathFinder erweitert. Diese beinhaltet zusätzlich, wie der Name schon sagt, das Clustering.
...
Listing \ref{lst:pathFinder.simple} zeigt auf, wie ein Pfad mittels Pfadsuche Simple gefunden wird. Ein SimplePathFinder wird mit der Karte initialisiert. Danach kann die Suche mit \texttt{pf.search()} gestartet werden. Als Parameter wird der Suchalgorithmus Strategy.Simple, der Startpunkt (Position der Ameise) und Endpunkt (Position des Futters), sowie die maximalen Pfadkosten (hier: 16) mitgegeben.
...
Das Clustering wird während dem ClusteringTask (siehe \ref{sec:module.Tasks}) ausgeführt, Dabei wird die Landkarte, wie bereits erwähnt, in sogenannte Clusters unterteilt. Auf der Abbildung \ref{fig.clusteredMap} wurde die Karte in 16 Clusters aufgeteilt.
...
Wir unterschieden zwischen den zwei Clusterarten \textit{Centered} und \textit{Corner}. Die Variante Corner wurde bereits im Vormodule ''Projekt 2`` implementiert, während die Variante Center im Laufe dieser Arbeit dazu kam. Folgendes Bild zeigt den Unterschied der Varianten. Corner generiert zwei Übergangspunkte plus eine Verbindung auf der Kante zwischen zwei Clusters. Die Variante Centered generiert nur einen Übergangspunkt in der Kantenmitte der aneinander grenzenden Clusters. Die Variante Centered hat den Vorteil, dass es weniger dichtes Pfadnetz gibt, da weniger Übergangspunkte erzeugt werden, aber sie hat auch den Nachteil, dass der gefundene Pfad nicht in jedem Fall der Kürzeste ist. Es empfiehlt sich, zusätzlich ein Pathsmoothing anzuwenden.

\begin{figure}[H]
\centering
\includegraphics[height=50mm]{91_bilder/clusterArten}
\caption[Vergleich der Clusterarten]{Vergleich der Clusterarten: Links der Typ \textit{Centered}, Rechts der Typ \textit{Corner}}
\label{fig.clusteredKinds}
\end{figure}

Nachfolgend wird erläutert, wie das Clustering vonstatten geht, verwendet wird die Custeringart Corner.

Nach dem Einteilen der Cluster werden für jeden Cluster und einen Nachbar-Cluster aus der Vierer-Nachbarschaft die Verbindungskanten berechnet. Dies kann natürlich nur für Clusters gemacht werden, die auf einem sichtbaren Teil der Landkarte liegen, was zu Beginn des Spiels nicht unbedingt gegeben ist. Deshalb wird der ClusteringTask in jedem Spielzug aufgerufen, in der Hoffnung, dass der Cluster komplett sichtbar ist. Sobald eine beliebige Seite eines Clusters berechnet ist, wird diese Aussenkante im Cluster und dem anliegenden Nachbarn gespeichert und nicht mehr neu berechnet.
...
Um unser Search-Framework zu komplettieren, bietet die Pfadsuche auch ein PathSmoothing, das ''Glätten`` eines Pfades an. Wie im Abschnitt Clustering schon erwähnt, kann es sein, dass ein Pfad, der vom HPA* Algorithmus gefunden wurde, nicht zwingend der Kürzeste ist. Abbildung \ref{fig.pathsmoothingOne} veranschaulicht, wie der gefundene Pfad von Cluster zu Cluster (weisse Markierung), stets über die vorberechneten Verbindungspunkte (blau) verläuft. Dies ist nicht der optimale Pfad, er kann mit PathSmoothing verkürzt werden.
