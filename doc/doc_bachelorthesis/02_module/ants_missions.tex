...
\\
Beim Entwicklen dieser Mission sind wir auf folgende Fragen gestossen: Ab welchem Spielzug soll der Hügel bewacht werden? Wieviel Ameisen sollen für die Verteidigung eingesetzt werden? Was wenn der Hügel von mehreren Seiten angegriffen wird? Sollen alle Hügel bewacht werden? Aus diesen Frage ergab sich folgende Implementation: \\
\\
Die DefendHillMission bietet zwei Modi zur Verteidigung an. Die Default-Verteidigung und der Barrier-Modus. Bei der Default-Verteidigung werden die Ameise nahe um den eigenen Hügel zur Verteidigung aufgestellt. Der Barrier-Modus ist nur bei Hügelmöglich die in einer Sackgasse sind, den in diesem Modus wird eine Sperre an der engsten Stelle erstellt. Nachfolgende Abbildung veranschaulicht die beiden Modi.
...
Hier wird jegentlich berechnet, wieviele Ameisen den Hügel angreifen. Die Angreifer und die Verteidiger werden dem DefendingCombatPositioning übergeben, dieses berechnet die Positionen der Ameisen. Es ist einstellbar ab welchem Zug die DefendMission Ameisen für die Verteidigung rekrutiert. Zudem kann mit dem Parameter textif{DEFENDER_MORETHAN_ATTACKERS} definiert werden wieviel Ameisen mehr als die Anzahl Angreifer rekrutiert werden. Ameisen werden von der Mission entlassen wenn weniger Angreifer als Veteidiger unter der Berücksichtigung des erwähnten Parameter gezählt werden.

\subsection{Barrier-Modus}
\label{subsec:module.Mission.DefendHillMission.BarrierModus}

Wie schon erwähnt, kann dieser Modus nur eintreffen wenn die Geländegegebenheiten stimmen. Ziel war es das Engpässe auszunutzen um mit möglichst wenigen Ameisen zu verteidigen. Eine weitere Idee war, dass durch die Sperre der Gegners die Umgebung dahinter nicht erkunden kann, so unseren Hügel nie zu Gesicht bekommt und kein Angriff auf unseren Hügel startet. Leider verbesserte diese Method die Fähigkeiten nur gering, den die Ameisen welche für die Sperre benötigt werden, müssen entsprechend platziert werden und können nicht für andere Aufgaben verwendet werden. Zudem hat die Sperre bei starken Gegnern, wie Xathis oder Greentea, nut gerning Wirkung. Diese Bot warten vor der Sperre bis sie in Überzahl sind und überrumpeln dann unsere Verteidigung.

\begin{figure}[H]
\centering
\includegraphics[height=40mm]{91_bilder/BarrierOverTaken}
\caption{Eine Ameise auf einer ExploreMission bewegt sich in Richtung Fog Of War}
\label{fig:BarrierOverTaken}
\end{figure}

Die Funktionswiese ist wie folgt. Es werden je nach Grösse der Sperre entsprechend viele Ameisen zur Position geschickt wo die Sperre aufgebaut ist. Die Ameisen stellen sich paarweise hintereinander auf, solange keine Gegner in der Nähe sind. So können Ameisen die einer anderen Aufgabe nachgehen die Sperre passieren. Wenn der Gegner sich in Überzahl nähert, wird die Sperre geschlossen. Danach wird das Kampfverhalten dem AttackingCombatPositioning überlassen, welches die Positionierung übernimmt.

\begin{figure}[H]
\centering
\includegraphics[height=40mm]{91_bilder/CloseBarrier}
\caption{Eine Ameise auf einer ExploreMission bewegt sich in Richtung Fog Of War}
