...
\paragraph{Strategy:} Das Strategy-Module enthält strategische und taktische Algorithmen, insbesondere die Influence Map und das CombatPositioning. Die Ausführliche Dokumentation  befindet sich im Kapitel \ref{sec:module.StrategieTaktik}.
...
\paragraph{TestReader:} Das TestReader-Modul besteht aus einer einzelnen Klasse, die aus den Log-Files der Ants-Spielengine die Spielauswertungen lesen kann. Dies im Kapitel \ref{sec:module.Testreader} ist genauer beschrieben.
...
\item
\textbf{JQuery \url{http://jquery.com/}:} Die JavaScript Library haben wir verwendet, um uns die zusätzlichen Informationen auf dem Spielfeld anzuzeigen.
\end{itemize}